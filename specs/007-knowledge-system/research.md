...
**Decision**: Snapshot is a single Markdown file generated by the CLI (`npx knowledge-system refresh`). Scanners read each artifact source (conventions from `.agreements/`, ADRs from `.adr/`, features from `.features/`), extract title + status, and write structured sections.
...
- `/knowledge.refresh` — wrapper that calls `npx knowledge-system refresh`
- `/knowledge.check` — wrapper that calls `npx knowledge-system check` + interprets results
...
- `installer.js`: Add `{ name: "Knowledge System", flag: "--skip-knowledge", cmd: "npx knowledge-system init" }`
- `updater.js`: Add `{ name: "Knowledge System", marker: ".knowledge", pkg: "knowledge-system", cmd: "npx knowledge-system update" }`
