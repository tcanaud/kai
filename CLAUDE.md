...
- File-based — `.product/` directory tree with YAML frontmatter + Markdown body files; `index.yaml` as computed summary (018-cli-kai-product)
...
## Recent Changes
- 018-cli-kai-product: Added Node.js ESM (`"type": "module"`), Node >= 18.0.0 + None — zero runtime dependencies (Node.js built-ins only via `node:` protocol imports)
